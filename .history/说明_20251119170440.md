...
实现了内存安全的 MJPEG 流媒体服务，用于实时测试可视化。利用 Playwright 的掩码选项进行预捕获数据编辑和
...
2. 指标提取与重构
根据你提供的原文，我们可以提取出更具体、更专业的指标和描述。
条目一：并发控制
原始问题：并发测试负载是多少？时延是多少？
分析：你的原文描述了一个基于内存、CPU负载、事件循环延迟三个维度的动态并发控制器。这是一个很好的设计，但简历里完全没体现出来。
可提取的指标：
约束条件：内存使用率上限（85%），CPU负载率上限（90%），事件循环延迟上限（不合理，因为降级时并发数会迅速下降）。
性能结果：在高负载下（例如，模拟每秒入队10个任务），系统的平均任务吞吐量是多少（例如 5 tasks/sec），以及任务平均等待时间是多少（例如 2.5s）。
