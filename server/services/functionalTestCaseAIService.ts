...
- **SMOKE（冒烟用例）**【必须生成至少1条，优先级必须为 high】：功能验证、基本正向流程、高优先级场景、主要业务路径、必须通过的基础功能
  * 示例：用户登录的正常流程、订单创建的基本流程、数据查询的主要功能
  * 判断标准：如果该功能不通过，整个模块基本不可用
  * 🔥 **SMOKE 用例的 priority 字段必须设置为 "high"**
  
- **FULL（全量用例）**：详细功能验证、边界条件测试、异常情况处理、各种输入组合、非功能验证
- **ABNORMAL（异常用例）**：异常情况处理、各种输入组合、非功能验证
- **BOUNDARY（边界用例）**：边界条件测试、各种输入组合、非功能验证
- **PERFORMANCE（性能用例）**：性能测试、各种输入组合、非功能验证
- **SECURITY（安全用例）**：安全测试、各种输入组合、非功能验证
- **USABILITY（可用性用例）**：可用性测试、各种输入组合、非功能验证
- **COMPATIBILITY（兼容性用例）**：兼容性测试、各种输入组合、非功能验证
- **RELIABILITY（可靠性用例）**：可靠性测试、各种输入组合、非功能验证
...
2. **用例名称必须反映测试点的场景** - 例如测试点是"用户名为空"，用例名也应该是"用户名为空"相关
3. **根据测试点性质选择正确的用例类型**：
   - 测试点是正常流程 → SMOKE
   - 测试点是边界条件 → BOUNDARY
   - 测试点是异常情况 → ABNORMAL
...
直接输出JSON格式，不要其他说明文字。`;
  ...
  private determineCaseType(tc: any, testPoint: TestPoint): string {
    ...
    const isCoreFunction = this.containsKeywords(combinedText, ['', '基本', '主流程', '关键', '必须', '登录', '注册']);
...
- SMOKE 用例用于验证功能的基本正向流程
- 如果原始用例中有 SMOKE 类型，必须保留
- 如果原始用例中没有 SMOKE 类型，需要将最的用例设置为 SMOKE
