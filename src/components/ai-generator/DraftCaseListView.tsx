...
/**
 * 紧凑列表视图 - 列顺序与表格视图一致，支持全选
 */
export function DraftCaseListView({
  draftCases,
  selectedTestCases,
  onToggleSelect,
  onViewDetail,
  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white">
      ...
      {/* 表头 */}
      <div className="flex items-center gap-3 px-3 py-2 bg-gradient-to-r from-gray-50 to-purple-50/30 border-b border-gray-200 text-xs font-semibold text-gray-500 uppercase tracking-wider">
        ...
        <span className="w-[150px]">测试场景</span>
        <span className="w-[150px]">测试点</span>
        ...
        <span className="w-[44px] text-center">步骤</span>
        <span className="w-[50px] text-center">质量</span>
        <span className="w-[56px] text-center">状态</span>
      ...
      {filteredAndSortedCases.map((tc, index) => {
        ...
        return (
          ...
          >
            ...
            {/* 序号 - 使用简单的递增序号 */}
            <span className="w-[20px] text-center text-gray-400 font-mono">
              {startIndex + index + 1}
            </span>

            {/* 测试场景 */}
            <Tooltip title={tc.scenarioName}>
              <span className="w-[150px] text-gray-500 truncate block">
                {tc.scenarioName || '-'}
              </span>
            </Tooltip>
            ...
            {/* 类型 */}
            <span className={clsx(
              "w-[60px] text-center px-1.5 py-0.5 rounded text-[10px] font-semibold border whitespace-nowrap",
            ...
            {/* 优先级 */}
            <div className="w-[56px] flex items-center justify-center gap-1">
              <div className={clsx("w-2 h-2 rounded-full", priority.dot)} />
              <span className={clsx("font-medium whitespace-nowrap", priority.color)}>{priority.label}</span>
            </div>

            {/* 步骤 */}
            <div className="w-[44px] flex items-center justify-center gap-1">
              <List className="w-3 h-3 text-gray-400" />
              <span className="text-gray-600">{countSteps(tc.steps)}</span>
            </div>

            {/* 质量 */}
            <div className="w-[50px] flex items-center justify-center gap-1">
              <Star className="w-3 h-3 text-yellow-400 fill-yellow-400" />
              <span className="font-semibold text-gray-600">{tc.qualityScore || 85}</span>
            </div>
            ...
            <div className="w-[56px] text-center">
              {saved ? (
                ...
                <span className="text-[10px] font-medium text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded">草稿</span>
              )}
            </div>
            ...
            <button
              ...
              className="w-[40px] flex justify-center p-1 rounded text-gray-400 hover:text-purple-600 hover:bg-purple-50 transition-all"
            >
